...
from utils.api_monetar import get_agregate_monetare_bns, get_credite_depozite_bns
...
def render():
    ...
    with tab2:
        ...
        # Tabel sinteza
        st.markdown('<div class="chart-card"><div class="chart-card-title">Tabelul indicatorilor monetari</div>', unsafe_allow_html=True)
        cols_tbl = {
            "an":                    "An",
            "ipc_sfarsit_an":        "IPC sfarsit an (%)",
            "ipc_medie_an":          "IPC medie anuala (%)",
            "rezerve_mil_eur":       "Active oficiale de rezerva BNM (mil. EUR)",
    ...
    with tab1:
        ...
        with col1:
            ...
            if mo_col in df.columns:
                fig5 = go.Figure(go.Scatter(
                    x=YEARS_STR, y=df[mo_col].tolist(),
                    mode="lines+markers",
        ...
        # mo_tbl_col = "bani_lichizi_mo_mil_lei" if "bani_lichizi_mo_mil_lei" in df.columns else "baza_monetara_mil_lei"
        # cols_b = {c: c for c in ["an", mo_tbl_col, dep_col] if c in df.columns}
        # tbl_b = df[list(cols_b)].rename(columns={
        #     "an": "An",
        #     "bani_lichizi_mo_mil_lei": "MO — Bani lichizi (mil. lei)",
        ...
        if all(c in df.columns for c in dep_struct) and "depozite_total_mil_lei" in df.columns:
            ...
            for col_k, name, color in [
                ...
                fig_dpct.add_trace(go.Bar(
                    ...
                    hovertemplate=f"<b>%{{x}}</b> {name}: %{{y:.2f}}%<extra></extra>",
                ))
        ...
        dep_rename = {
            "an":                         "An",
            "bani_lichizi_mo_mil_lei":      "Bani lichizi MO",
            "depozite_total_mil_lei":      "Total depozite",
    ...
    with tab3:
        ...
        with col1:
            st.markdown('<div class="chart-card"><div class="chart-card-title">Credite noi acordate persoanelor juridice si fizice (mil. lei)</div>', unsafe_allow_html=True)
            fig_cr = go.Figure()
            ...
            if "credite_fizice_mil_lei" in df.columns:
                fig_cr.add_trace(go.Bar(
                    ...
                    marker_color="#5FC8EE", opacity=0.85,
        ...
        st.markdown('<div class="chart-card"><div class="chart-card-title">Depozite noi la termen atrase persoane juridice si fizice (mil. lei)</div>', unsafe_allow_html=True)
        fig_dt = go.Figure()
        ...
        # Tabel credite
        st.markdown('<div class="chart-card"><div class="chart-card-title">Datele privind creditele noi acordate (mil. lei)</div>', unsafe_allow_html=True)
        cr_rename = {
            "an": "An",
